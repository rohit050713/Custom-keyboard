package com.www.customkeyboard.Fragments;

<<<<<<< HEAD
import android.content.Context;
=======
>>>>>>> 02bb1b856fec1d032aa22613a7c3b54da93eb1d7
import android.os.Bundle;

import androidx.fragment.app.Fragment;

import android.view.LayoutInflater;
import android.view.View;
import android.view.ViewGroup;
<<<<<<< HEAD
import android.widget.ImageView;

import com.www.customkeyboard.R;


public class CustomThemeFragment extends Fragment {

ImageView back;
Context context;
    public CustomThemeFragment(Context context) {
        this.context=context;
    }




=======

import com.www.customkeyboard.R;

/**
 * A simple {@link Fragment} subclass.
 * Use the {@link CustomThemeFragment#newInstance} factory method to
 * create an instance of this fragment.
 */
public class CustomThemeFragment extends Fragment {

    // TODO: Rename parameter arguments, choose names that match
    // the fragment initialization parameters, e.g. ARG_ITEM_NUMBER
    private static final String ARG_PARAM1 = "param1";
    private static final String ARG_PARAM2 = "param2";

    // TODO: Rename and change types of parameters
    private String mParam1;
    private String mParam2;

    public CustomThemeFragment() {
        // Required empty public constructor
    }

    /**
     * Use this factory method to create a new instance of
     * this fragment using the provided parameters.
     *
     * @param param1 Parameter 1.
     * @param param2 Parameter 2.
     * @return A new instance of fragment CustomThemeFragment.
     */
    // TODO: Rename and change types and number of parameters
    public static CustomThemeFragment newInstance(String param1, String param2) {
        CustomThemeFragment fragment = new CustomThemeFragment();
        Bundle args = new Bundle();
        args.putString(ARG_PARAM1, param1);
        args.putString(ARG_PARAM2, param2);
        fragment.setArguments(args);
        return fragment;
    }

    @Override
    public void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        if (getArguments() != null) {
            mParam1 = getArguments().getString(ARG_PARAM1);
            mParam2 = getArguments().getString(ARG_PARAM2);
        }
    }
>>>>>>> 02bb1b856fec1d032aa22613a7c3b54da93eb1d7

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        // Inflate the layout for this fragment
<<<<<<< HEAD
        View view= inflater.inflate(R.layout.fragment_custom_theme, container, false);
         back=view.findViewById(R.id.iv_custom_themes_back);
         back.setOnClickListener(new View.OnClickListener() {
             @Override
             public void onClick(View v) {
                 getFragmentManager().beginTransaction().replace(R.id.frame_container,new ProfileFragment(context)).commit();
             }
         });

        return view;
=======
        return inflater.inflate(R.layout.fragment_custom_theme, container, false);
>>>>>>> 02bb1b856fec1d032aa22613a7c3b54da93eb1d7
    }
}